...
def get_mel_image_from_int32(
    audio_32, max_len=40, n_mfcc=28, sound_rate=PLAYREAD_SPEED
):
...
def normalize_mfcc(mfcc, min_val=-1, max_val=1):
    ...
    return (
        (mfcc - mfcc_min) * (max_val - min_val) / (mfcc_max - mfcc_min) + min_val
    ).astype(np.float32)


def get_mel_image_from_float_normalized(
    audio_float, max_len=40, n_mfcc=28, sound_rate=PLAYREAD_SPEED
):
            ...
            try:
                ...
                sp.call(
                    [
                        "ffmpeg",
                        "-i",
                        str(os.path.join(path, audio_file)),
                        str(os.path.join(os.getcwd(), dest, str(i) + ".wav")),
                    ]
                )
                i += 1
...
def GetAudioDataSegment(filename, currentTime, sampleSize):
    ...
    command = [
        ...
        "pcm_s16le",
        "-ar",
        str(PLAYREAD_SPEED),  # ouput will have 44100 Hz
        "-ac",
        "2",  # stereo (set to '1' for mono)
        "-ss",
        str(timedelta(seconds=currentTime)),
        "-to",
        str(timedelta(seconds=currentTime + sampleSize)),
        "-",
    ]
