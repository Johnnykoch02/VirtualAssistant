...
class Gwen:
    ...
    class Context:
        ...
        def exec(self, kwargs):
            ...
            self._exec_thr = thr.Thread(
                target=self.obj.exec, args=(self.data), daemon=True
            )
    ...
    class SpotifyContext(Context):
        ...
        def exec(self, kwargs):
            if kwargs["func"] == "play":
                self.obj.search(
                    (kwargs["song"]) + " " + kwargs.get("artist", "").strip()
                )
        ...
        def validate_exec(self, kwargs) -> bool:
            ...
            return (
                kwargs["func"] == "play"
                and kwargs.get("song", None) != None
                and kwargs.get("artist", "") != None
                or kwargs["func"] == "pause"
            )
    ...
    class YouTubeContext(Context):
        ...
        def exec(self, kwargs):
            self.obj.play(
                kwargs["query"], channel_name=kwargs.get("channel_name", None)
            )
    ...
    class GwenContext(Context):
        ...
        def run(self, is_main_context=False) -> None:  # TODO: Actually implement this
            if (
                not "clear_context" in self.data["target"]
                and not "output_speech" in self.data["target"]
            ):
        ...
        def validate_exec(self, kwargs) -> bool:
            assert isinstance(kwargs, dict)
            return (
                kwargs["func"] == "output_speech"
                and kwargs.get("text", None) != None
                or kwargs["func"] == "clear_context"
                or kwargs["func"] == "collect_keyword_data"
                and kwargs.get("num_samples", None) != None
                and isinstance(kwargs["num_samples"], int)
            )
    ...
    def run_context(
        ...
        [
            context.run(is_main_context=current_context == context)
