...
class NonLSTMKeywordAudioModel(nn.Module):
    ...
    def __init__(
        ...
        self._ConvolutionalLayers = nn.Sequential(
            ...
            nn.MaxPool2d(2, 2),
            nn.Conv2d(
                in_channels=64, out_channels=128, kernel_size=(2, 2), stride=2
            ),  # Output (20 x 9 x 8)
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Flatten(),
...
class LSTMKeywordAudioModel(nn.Module):
    ...
    def __init__(
        ...
        self._ConvolutionalLayers = nn.Sequential(
            nn.Conv2d(
                in_channels=1, out_channels=32, kernel_size=(8, 8), padding="same"
            ),
            nn.LeakyReLU(),
            nn.Conv2d(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(
                in_channels=64, out_channels=64, kernel_size=(4, 4), stride=2, padding=0
            ),
            nn.LeakyReLU(),
            nn.MaxPool2d((3, 3)),
    ...
    def forward(self, x, hidden=None):  # TODO: Fix this and Implement Hidden State :3
        ...
        x, hidden = self.lstm(
            x.view(batch_size, num_timesteps, -1),
            (th.nan_to_num(hidden[0]), th.nan_to_num(hidden[1])),
        )
        x = self.fc(x)
        self._hidden = hidden
        return x
    ...
    def init_hidden(self, batch_size):
        hidden_state = th.zeros(self.lstm_layers, batch_size, self.lstm_hidden_size).to(
            self.device
        )
        cell_state = th.zeros(self.lstm_layers, batch_size, self.lstm_hidden_size).to(
            self.device
        )
