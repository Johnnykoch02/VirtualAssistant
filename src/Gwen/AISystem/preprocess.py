...
Data_Path = os.path.join(os.getcwd(), "data/Models/SpeechModel/Training/")
...
def audioDataTomfcc(sound, max_len=64, n_mfcc=72):
    ...
    data_s16 = np.frombuffer(
        byte_data, dtype=np.int32, count=len(byte_data) // 2, offset=0
    )
...
def save_data_to_array(path, max_len=64, n_mfcc=72):
    ...
    for label in labels:
        ...
        wavfiles = [
            os.path.join(path, "Audio", label, wavfile)
            for wavfile in os.listdir(os.path.join(path, "Audio", label))
        ]
...
def get_train_test(split_ratio=0.6, random_state=42):
    ...
    return train_test_split(
        X, y, test_size=(1 - split_ratio), random_state=random_state, shuffle=True
    )
...
def prepare_dataset(path=Data_Path):
    ...
    for label in labels:
        data[label] = {}
        data[label]["path"] = [
            path + label + "/" + wavfile for wavfile in os.listdir(path + "/" + label)
        ]
