...
class CommandController(object):
    def __init__(self, GwenInstance):
        ...
        self._config = get_json_variables(
            os.path.join(
                os.getcwd(), "data", "Gwen", "Backend", "CommandControllerConfig.json"
            ),
            ["prompt_path", "context", "command", "bad_data_prompt"],
        )
    ...
    def ProcessCommand(self, command, context):
        ...
        """
        prompt = self._gpt_prompt.replace(self._context, str(context)).replace(
        ...
        for _ in range(3):
            try:
                ...
                cmd_kwargs = {
                    key: value for key, value in backend_cmd.items() if key != "target"
                }
                cmd_kwargs["func"] = func
                context = self.ContextReferences[context_class](
                    backend_cmd,
                )
                if context.validate_exec(cmd_kwargs):
                    context.exec(cmd_kwargs)  # Execute the context.
                    s = True
                    self.GwenInstance.add_context(
                        context
                    )  # Add the current context to the GwenInstance.
